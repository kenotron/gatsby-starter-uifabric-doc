...
dos: |-
  ...
  - Group multipled similar events occuring near the same time into single ActivityItems.
donts: |-
  - Use ActivityItemds to render large amounts of body text; they are meant to be concise descriptions of a specific activity.
overview: |-
  Toggles represent a dphysical switch that allows users to turn things on or off. Use Toggles to present users with two mutually exclusive options (like on/off), where choosing an option results in an immediate action. Use a Toggle for binary operations that take effect right after the user flips the Toggle. For example, use a Toggle to turn services or hardware components on or off. In other words, if a physical switch would work for the action, a Toggle is probably the best control to use.
  ...
  For some actions, either a Toggle or a Checkbox might work. To decide which control would work better, follow these tips:
    ...
    - Use a Checkbox when the user has to perform extra steps for changes to be effective. For example, if the user smust click a "Submit", "Next", "Ok" button to apply changes, use a Checkbox.
---
...
```js live noInline
const className = mergeStyles({ background: 'red' });
